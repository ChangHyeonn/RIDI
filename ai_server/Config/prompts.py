...
- **시간 정보 제공**: "오후 5시야", "3시", "오전 9시" → schedule_add (이전 일정 추가 맥락에서)**
- **시간 보완 응답**: "~시야", "~시에", "~시로" → schedule_add (누락된 시간 정보 제공)**
...
**연속 대화 예시 (시간 정보 제공):**
- **1차**: "이번 주 토요일에 친구랑 영화 보기로 했어" → schedule_add (시간 누락)
- **2차**: "오후 5시야" → schedule_add (시간 정보 제공)
- **1차**: "내일 친구 만나기로 했어" → schedule_add (시간 누락)
- **2차**: "3시에" → schedule_add (시간 정보 제공)
- **1차**: "다음 주 월요일에 회의 있어" → schedule_add (시간 누락)
- **2차**: "오전 9시" → schedule_add (시간 정보 제공)
...
**일정 조회 요청 감지 규칙:**
- "~보여줘", "~알려줘", "~언제" → schedule_read (키워드 검색)
- "~관련", "~이 언제" → schedule_read (키워드 검색)
- "내일 일정", "다음주 일정", "특정 날짜 일정" → schedule_read (날짜별 조회)
- "일정 목록", "모든 일정", "일정 보여줘" → schedule_read (전체 조회)
- "중요한 일정", "중요 일정" → schedule_read (중요 일정 조회)
- "~이 언제 있어?" (존재 여부만 묻는 경우) → schedule_read
- **"~로 한 일정" → schedule_read (description 기반 검색)**
- **"~일정 있으면 찾아" → schedule_read (description 기반 검색)**
- **"~일정 찾아" → schedule_read (description 기반 검색)**
...
- **예: "친구랑 롯데시네마에서 영화 보기로 한 일정" → "친구랑 롯데시네마에서 영화 보기" 부분을 description과 비교하여 검색**
...
5. **location 필드는 비워두고**: 모든 장소 정보는 description에 포함
...
{{
    ...
    "location": null,
    "description": "추가 설명 (장소, 사람, 구체적인 활동 포함)",
...
- "회의 일정" → keyword: "회의"
- "운동 일정" → keyword: "운동"
- **"친구랑 롯데시네마에서 영화 보기로 한 일정" → keyword: "친구랑 롯데시네마에서 영화 보기"**
- **"강남역에서 친구 만나서 커피 마신 일정" → keyword: "강남역에서 친구 만나서 커피"**

**⚠️ 중요: "일정"이라는 단어는 절대 키워드로 추출하지 마세요!**
- "~일정" → "일정" 제외하고 앞부분만 키워드로 추출
- "~로 한 일정" → "~로 한" 부분을 키워드로 추출
- "~관련 일정" → "~관련" 부분을 키워드로 추출
- **"~일정 있으면 찾아" → "~" 부분을 키워드로 추출**
- **"~일정 찾아" → "~" 부분을 키워드로 추출**

**🚨 절대 금지:**
- "일정"이라는 단어를 키워드로 사용하는 것
- "일정"이라는 단어만 추출하는 것
- "일정"이라는 단어를 포함한 키워드 추출

**핵심 키워드 추출 원칙:**
- **장소 + 활동**: "롯데시네마 영화", "강남역 커피"
- **핵심 장소 + 핵심 활동**: "이월드 놀기", "병원 진료", "치과 검진"
- **핵심 단어 조합**: "친구 이월드", "친구 영화", "친구 커피"
- **불필요한 단어 제거**: "가서", "로 한", "있으면" 등 제거
- **2-3개 핵심 단어 조합**: 너무 긴 키워드는 매칭 실패 가능성 높음
- **복수 키워드 우선**: 단일 키워드보다는 2-3개 키워드 조합이 더 정확함

**❌ 잘못된 예시:**
- "친구랑 롯데시네마에서 영화 보기로 한 일정" → keyword: "일정" (❌)
- "병원 관련 일정" → keyword: "일정" (❌)
- **"친구랑 이월드 가서 놀기로 한 일정 있으면 찾아" → keyword: "일정" (❌)**

**✅ 올바른 예시:**
- "친구랑 롯데시네마에서 영화 보기로 한 일정" → keyword: "친구 롯데시네마 영화" (✅)
- "병원 관련 일정" → keyword: "병원 관련" (✅)
- **"친구랑 이월드 가서 놀기로 한 일정 있으면 찾아" → keyword: "친구 이월드 놀기" (✅)**
...
- "롯데시네마에서 영화 본 일정" → type: "keyword", keyword: "롯데시네마에서 영화 본" (description에서 검색)
- "친구랑 커피 마신 일정" → type: "keyword", keyword: "친구랑 커피 마신" (description에서 검색)
- "강남역에서 만난 일정" → type: "keyword", keyword: "강남역에서 만난" (description에서 검색)
- **"친구랑 롯데시네마에서 영화 보기로 한 일정" → type: "keyword", keyword: "친구 롯데시네마 영화"**
- **"강남역에서 친구 만나서 커피 마신 일정" → type: "keyword", keyword: "친구 강남역 커피"**
- **"친구랑 이월드 가서 놀기로 한 일정 있으면 찾아" → type: "keyword", keyword: "친구 이월드 놀기"**
- **"이월드에서 놀기로 한 일정 찾아" → type: "keyword", keyword: "이월드 놀기"**
...
**일정 조회 (주제+날짜/시간대 결합):**
- "다음주 수요일에 있는 친구랑 영화 보러가는 일정" → type: "keyword", keyword: "친구랑 영화 보러가는", date: "다음주 수요일 날짜"
...
- "이번 주 토요일 저녁 친구 만남 일정" → type: "keyword", keyword: "친구 만남", date: "2025-08-23", time_period: "evening"
- **"친구랑 롯데시네마에서 영화 보기로 한 일정" → type: "keyword", keyword: "친구랑 롯데시네마에서 영화 보기"**
- **"강남역에서 친구 만나서 커피 마신 일정" → type: "keyword", keyword: "강남역에서 친구 만나서 커피"**
